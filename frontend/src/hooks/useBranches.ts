...
  
  // Location Object ถูกรวมอยู่ใน branch แล้ว
...
  const randomColor = colors[(branch.id || 0) % colors.length]; 

  // ฟังก์ชันเล็ก ๆ ไว้รวมชื่อกับนามสกุลเป็นชื่อเต็ม
  const getFullName = (user: any): string | null => {
    if (!user) return null;
    // สมมติว่าชื่อผู้ใช้อยู่ในฟิลด์ firstName, lastName 
...
    ownerName:
      getFullName(branch.supervisor) || 
      getFullName(branch.sales) || 
      null,
    salesName: getFullName(branch.sales) || null,
    supervisorName: getFullName(branch.supervisor) || null,
    
    // ✅ ใช้ Optional Chaining กับ Relation Object ใหม่ (sales, supervisor)
...
    color: randomColor,
    
    // ✅ ใช้ Optional Chaining กับ location properties
...
      // ... (โค้ดจัดการ Stub และ Error เดิม) ...
...
      // 🚨 หาก Backend ส่งข้อมูลเป็น GeoJSON (properties) ต้องแก้ตรงนี้:
